...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $message = 'Jadwal berhasil ditambahkan! (Demo mode - data tidak tersimpan)';
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Tambah Jadwal - <?= SITE_NAME ?></title>
    ...
    <nav class="bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg">
        <div class="container mx-auto px-6 py-4">
            <div class="flex justify-between items-center">
                <h1 class="text-xl font-bold">Admin Panel - Tambah Jadwal</h1>
    ...
    <div class="flex">
        <aside class="w-64 bg-white shadow-lg min-h-screen">
            <nav class="p-6">
                ...
                <a href="add-schedule.php" class="flex items-center gap-3 px-4 py-3 bg-blue-100 text-blue-600 rounded-lg mb-2 font-semibold">
        ...
        <main class="flex-1 p-8">
            <h1 class="text-3xl font-bold text-gray-800 mb-8">Tambah Jadwal Baru</h1>
                ...
                <form method="POST" class="space-y-6">
                    <div class="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="title" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="date" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Waktu Mulai *</label>
                            <input type="time" name="time_start" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Waktu Selesai *</label>
                            <input type="time" name="time_end" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Lokasi *</label>
                            <input type="text" name="location" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Pembicara/PIC *</label>
                            <input type="text" name="speaker" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi *</label>
                        <textarea name="description" rows="5" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
